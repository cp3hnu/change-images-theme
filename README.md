...
一个用于按颜色映射表批量替换 PNG/JPG 像素颜色的 Node.js CLI 工具，针对 **品牌色重塑（rebrand）** 场景优化：能跨越同一品牌色的明度/饱和度变体，把所有"同色相"的像素整体迁移到新的色相。
...
```bash
# 只换非常接近源色相的像素（保守）
change-image-color -i in.png -o out.png -m map.json --hue-radius 15

# 默认：能覆盖品牌色变体，不波及邻近色相
change-image-color -i in.png -o out.png -m map.json

# 连邻近色相也调换（宽松）
change-image-color -i in.png -o out.png -m map.json --hue-radius 60
```
...
```bash
change-image-color -i in.png -o out.png -m map.json --saturation-threshold 0.05
```

特殊场景：如果你确实要把白/灰也换掉，用 `--no-preserve-neutrals`：

```bash
change-image-color -i in.png -o out.png -m map.json --no-preserve-neutrals
```

### 内联 JSON 映射

```bash
change-image-color -i in.png -o out.png -m '{"#514cf9":"#f05416"}'
```

### 详细输出

```bash
change-image-color -i in.png -o out.png -m map.json -v
```
